...
load("Growth_env_data_obs.Rdata")
...
IDCounts <- table(c(LengthAgeObs$RunningID, LWEnvData$RunningID, SRData$RunningID, LHTStaged$RunningID))
IDsMulti <- names(IDCounts)[which(IDCounts == 4)]
LHTMulti <- LengthAgeObs %>% filter(RunningID %in% IDsMulti) %>% drop_na(Age, fStage, fSex) %>% 
...
# For the variable importance analysis, this step would need to be repeated excluding one env. covariate at a time from the full model, saving the results of each run to the output folder.
